...
class MainWindow(QMainWindow):
    ...
    def check_mistakes(self, bit_str):
        ...
        position = 0
        for i in range(0, 6):
            position += int(cbits[i]) * self.control_bits[-1 * i - 1]
        if position == 0:
            if self.pbit == 0:
                self.outputFRAME(bit_str, "нет ошибок", 0, cbits)
            else:
                self.outputFRAME(bit_str, "ошибка p-bit", 43, cbits)
        else:
            if self.pbit == 0:
                self.outputFRAME(bit_str, "двойная ошибка", 0, cbits)
                return -1
            else:
                self.outputFRAME(bit_str, "Позиция ошибки: {}".format(str(position)), position, cbits)
                bit_str = self.change_el(bit_str, position)
    ...
    def outputFRAME(self, bit_str, message, error_pos, cbits):
        for i in range(-43, 0):
            if -1 * i in self.control_bits or i == -43:
                self.ui.CD_Message.setTextBackgroundColor(Qt.yellow)
            if -1 * i == error_pos:
                self.ui.CD_Message.setTextColor('#ff00ff')
    ...
    def add_mistakes(self, bit_str):
        ...
        if rand:
            ...
            if rand2 in range(1, 3):
                rand2 = rand
                while rand == rand2:
                    rand2 = random.randint(1, 43)
    ...
    def readComPort(self):
        try:
            ...
            while symbol != b'':
                ...
                elif symbol in check_symb:
                    self.bits_to_remove = int.from_bytes(symbol, byteorder='big')
                else:
                    symbol = symbol.decode('cp1251')
                    self.buf_to_get += str(symbol)
                    if len(self.buf_to_get) == 43:
                        self.ui.CD_Message.append("Фрейм: ")
